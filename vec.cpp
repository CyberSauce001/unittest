...
int main(int argc, char *argv[])
{
        ...
        printf("Alexander Nguyen                           \n");
        printf("Unit-test of vecNormalize function.        \n");
        printf("Tolerance: 1.000000                        \n\n");
        printf("     ----------------  --------------------------  \n");
...
-7.3129,    -6.6919,    9.9126,     -0.7377,    -0.6751,
...
void unitTest_normalize()
{
    for (int i=0; i<266; i++) {
        ...
        tmp[3] = varr[i][3];
        tmp[4] = varr[i][4];
        Vec tmp2 = { varr[i][0] , varr[i][1] };
        Vec tmp3 = { tmp2[0], tmp2[1]}; 
        Flt tolerance = 1.000000;
        Flt ret = vecNormalize(tmp3);
        //Flt ret = vecNormalize(tmp2);
        Flt res = ret - varr[i][2];
        bool test = false;
        printf("Debug ret: %lf %lf %lf\n\n",ret, tmp3[0], tmp3[1]); 
        if (tmp3[1] > varr[i][4] && tmp3[0] > varr[i][3]) {
                printf("%3i %7.4lf %7.4lf %8.4lf %7.4lf %7.4lf", i+1, varr[i][0],
                    varr[i][1], ret, varr[i][3], varr[i][4]);
                printf("<------------error in Y or X\n");
                test = true;
         
        }
        if (ret >= varr[i][2] && res <= tolerance && test != true) {
                printf("%3i %7.4lf %7.4lf %8.4lf %7.4lf %7.4lf\n", i+1, varr[i][0],
                    varr[i][1], ret, varr[i][3], varr[i][4]);
         
        }else if (!test) {
           printf("%3i %7.4lf %7.4lf %8.4lf %7.4lf %7.4lf\n", i+1, varr[i][0],
                varr[i][1], varr[i][2], varr[i][3], varr[i][4]);
        }
    }
}
